...
const FarmPage: React.FC = () => {
  ...
  const [actionPlanContent, setActionPlanContent] = useState<string | null>(null);

  const handleViewActionPlan = async () => {
    setShowActionPlan(true);
    setActionPlanLoading(true);
    setActionPlanContent(null);
    try {
      // ✅ FIXED: Call backend proxy instead of Anthropic directly.
      // The API key is stored securely on the server (ANTHROPIC_API_KEY env var).
      const response = await fetch('http://localhost:8000/ai/action-plan', {
        method: 'POST',
      ...
      setActionPlanContent(data.plan || (ar ? 'تعذر تحميل الخطة' : 'Failed to load plan'));
    } catch (e: any) {
      setActionPlanContent(
        ar
          ? `خطأ في الاتصال بالخادم: ${e?.message || 'خطأ غير معروف'}`
          : `Error connecting to AI server: ${e?.message || 'Unknown error'}`
      );
    } finally {
  ...
  return (
    <div className="animate-fadeInUp space-y-6 pb-8">
      ...
      {/* ══ ACTION PLAN MODAL ══ */}
      {showActionPlan && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={() => setShowActionPlan(false)}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(52,211,153,0.3)', borderRadius: 20, padding: 28, width: '100%', maxWidth: 560, maxHeight: '80vh', overflowY: 'auto', position: 'relative' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
              <div style={{ width: 36, height: 36, background: 'rgba(52,211,153,0.15)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              <div>
                <p style={{ fontSize: 15, fontWeight: 800, color: 'var(--text-primary)' }}>{ar ? 'خطة التدخل الذكية' : 'AI Intervention Action Plan'}</p>
                <p style={{ fontSize: 11, color: 'var(--emerald)', marginTop: 2 }}>🌾 {ar ? 'حقل الفراولة — رطوبة 29%' : 'Strawberry Field — 29% moisture'}</p>
              </div>
              <button onClick={() => setShowActionPlan(false)} style={{ marginInlineStart: 'auto', background: 'rgba(255,255,255,0.07)', border: 'none', borderRadius: 8, padding: '5px 9px', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: 16 }}>✕</button>
            </div>

            {actionPlanLoading ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, padding: '32px 0' }}>
                <div style={{ width: 40, height: 40, border: '3px solid rgba(52,211,153,0.2)', borderTopColor: 'var(--emerald)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
                <p style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{ar ? 'يقوم الذكاء الاصطناعي بتحليل الوضع...' : 'AI is analyzing the situation...'}</p>
              </div>
            ) : (
            ...
            {!actionPlanLoading && actionPlanContent && (
              <button onClick={() => setShowActionPlan(false)} style={{ marginTop: 20, width: '100%', background: 'rgba(52,211,153,0.12)', border: '1px solid rgba(52,211,153,0.3)', color: 'var(--emerald)', padding: '10px', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                {ar ? 'حسناً، مفهوم' : 'Got it, Close'}
              </button>
            )}
          </div>
        </div>
