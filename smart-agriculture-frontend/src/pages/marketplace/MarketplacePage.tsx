...
const ListingCard: React.FC<ListingCardProps> = ({ listing, ar, onBuy, canBuy, onAdminRemove }) => {
    ...
    <div className="card card-hover flex flex-col">
      ...
      <div className="px-5 pb-5">
        {canBuy ? (
          ...
          <button
            onClick={() => onAdminRemove(listing.id)}
            className="w-full py-2.5 rounded-xl text-sm font-bold transition-all"
            style={{ background: 'rgba(244,63,94,0.08)', border: '1px solid rgba(244,63,94,0.25)', color: '#f43f5e', cursor: 'pointer' }}
...
interface OrdersTableProps { orders: Order[]; ar: boolean; adminView?: boolean; onMarkComplete?: (id: string) => void; }
const OrdersTable: React.FC<OrdersTableProps> = ({ orders, ar, adminView, onMarkComplete }) => (
  <div className="card">
            ...
            <div key={order.id} className="px-6 py-4 flex items-center gap-4 hover:bg-white/[0.01] transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>
                  ...
                  {adminView && <> · {ar ? 'المشتري:' : 'Buyer:'} {order.buyerName} ({order.buyerRole})</>}
              ...
              <div className="flex flex-col items-end gap-2 flex-shrink-0">
                ...
                <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 999, background: scfg.bg, color: scfg.color, display: 'inline-block' }}>
                  {scfg.icon} {ar ? scfg.labelAr : scfg.labelEn}
                </span>
                {adminView && onMarkComplete && order.status === 'pending' && (
                  <button onClick={() => onMarkComplete(order.id)}
                    style={{ fontSize: 10, fontWeight: 800, padding: '3px 10px', borderRadius: 7, background: 'rgba(52,211,153,0.12)', border: '1px solid rgba(52,211,153,0.3)', color: 'var(--emerald)', cursor: 'pointer', whiteSpace: 'nowrap' }}>
...
const ListingsGrid: React.FC<ListingsGridProps> = ({ listings, ar, loading, canBuy, onBuy, onReset, page, totalPages, setPage, onAdminRemove }) => {
  if (loading) return (
  ...
  return (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {listings.map(l => <ListingCard key={l.id} listing={l} ar={ar} onBuy={onBuy} canBuy={canBuy} onAdminRemove={onAdminRemove} />)}
      </div>
      {totalPages > 1 && (
...
const RoleBanner: React.FC<{ role: string; ar: boolean }> = ({ role, ar }) => {
  const config: Record<string, { icon: string; color: string; bg: string; border: string; msgAr: string; msgEn: string }> = {
    ...
    trader:   { icon: '🏪', color: '#fbbf24', bg: 'rgba(251,191,36,0.06)', border: 'rgba(251,191,36,0.2)', msgAr: 'أنت تستخدم النظام كتاجر .', msgEn: "You're logged in as a Trader " },
...
const MarketplacePage: React.FC = () => {
  ...
  type AdminTab = 'browse' | 'allorders' | 'adminlistings';
  ...
  useEffect(() => {
    ...
    // Admin fetches all orders (including ones placed by traders in the same session)
    if (isAdmin) { marketplaceApi.getAllOrders().then(r => { if (r.success) setAllOrders(r.data as Order[]); }); }
  }, []);
  ...
  const handleBuy = async (qty: number) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        // Also push to allOrders so admin sees it in "All Orders" tab
        setAllOrders(prev => [res.data, ...prev]);
  ...
  // ── Add Listing (Admin) ──
  const handleAdminAddListing = async (payload: CreateListingPayload) => {
    setAddLoading(true);
    try {
      const res = await marketplaceApi.createListing(payload, 'admin-1', user?.name ?? 'أدمن', user?.name ?? 'Admin');
      if (res.success) {
        setShowAddModal(false);
        const newItem = res.data as Listing;
        setAdminListings(prev => [newItem, ...prev]);
        refreshStats();
        showToast(ar ? '✅ تم نشر المنتج بنجاح' : '✅ Item posted successfully!', 'success');
      }
    } catch { showToast(ar ? 'فشل نشر المنتج' : 'Failed to post item', 'error'); }
    finally { setAddLoading(false); }
  };
  ...
  const handleAdminDeleteListing = async (id: string) => {
    ...
    refreshStats();
    showToast(ar ? 'تم إزالة المنتج' : 'Item removed', 'info');
  };

  // ── Mark Order as Completed (Admin) ──
  const handleMarkOrderComplete = async (id: string) => {
    // Use completeOrder (sets status to 'delivered') instead of deleteOrder
    // so the trader still sees the order in their history with 'delivered' status
    await marketplaceApi.completeOrder(id);
    // Update admin's allOrders list: change status to delivered
    setAllOrders(prev => prev.map(o => o.id === id ? { ...o, status: 'delivered' as const } : o));
    // Also update the buyer's own orders list if it's loaded in the same session
    setOrders(prev => prev.map(o => o.id === id ? { ...o, status: 'delivered' as const } : o));
    showToast(ar ? '✅ تم تحديد الطلب كمكتمل — سيراه المشتري محدثاً' : '✅ Order marked complete — buyer will see updated status', 'success');
  };
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      {isAdmin && (
        <>
          {/* Admin Tabs */}
          <div className="flex flex-wrap gap-1 p-1 rounded-xl w-fit" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            {[
              ...
              { key: 'adminlistings', labelAr: `منتجاتي (${adminListings.length})`, labelEn: `My Listings (${adminListings.length})`, icon: '🛒' },
            ].map(t => (
          ...
          {adminTab === 'browse' && (
            <>
              ...
              <ListingsGrid listings={listings} ar={ar} loading={loadingListings} canBuy={false}
                ...
                onAdminRemove={handleAdminDeleteListing} />
            </>
          )}

          {adminTab === 'allorders' && <OrdersTable orders={allOrders} ar={ar} adminView={true} onMarkComplete={handleMarkOrderComplete} />}

          {adminTab === 'adminlistings' && (
            <>
              <div className="flex justify-end">
                <button onClick={() => setShowAddModal(true)}
                  className="px-5 py-2.5 rounded-xl text-sm font-bold"
                  style={{ background: 'linear-gradient(135deg,rgba(167,139,250,0.3),rgba(34,211,238,0.15))', color: '#a78bfa', border: '1px solid rgba(167,139,250,0.4)' }}>
                  ➕ {ar ? 'إضافة منتج للبيع' : 'Add Item for Sale'}
                </button>
              </div>
              {adminListings.length === 0 ? (
                <div className="card p-16 text-center">
                  <p className="text-5xl mb-4">🛒</p>
                  <p className="font-bold text-lg mb-2" style={{ color: 'var(--text-primary)' }}>
                    {ar ? 'لا توجد منتجات بعد' : 'No listings yet'}
                  </p>
                  <p className="text-sm mb-5" style={{ color: 'var(--text-muted)' }}>
                    {ar ? 'أضف منتجاً وابدأ البيع كمسؤول' : 'Add an item and start selling as admin'}
                  </p>
                  <button onClick={() => setShowAddModal(true)}
                    className="px-6 py-3 rounded-xl text-sm font-bold"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  {adminListings.map(l => (
                    <FarmerListingCard key={l.id} listing={l} ar={ar} onDelete={handleAdminDeleteListing} orders={[]} />
                  ))}
                </div>
              )}
            </>
      ...
      {showAddModal && (
        <AddListingModal ar={ar} onClose={() => setShowAddModal(false)} onSubmit={isAdmin ? handleAdminAddListing : handleAddListing} loading={addLoading} />
      )}
