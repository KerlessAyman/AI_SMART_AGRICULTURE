  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const html = `
        ...
        <head>
          <meta charset="UTF-8" />
          <title>${ar ? 'تقارير AgriMind AI' : 'AgriMind AI Reports'}</title>
          <style>
            body { font-family: Arial, sans-serif; color: #1a1a1a; background: #fff; margin: 0; padding: 24px; direction: ${ar ? 'rtl' : 'ltr'}; }
            h1 { color: #16a34a; border-bottom: 2px solid #16a34a; padding-bottom: 10px; font-size: 22px; }
            .meta { color: #666; font-size: 12px; margin-bottom: 24px; }
            .report { border: 1px solid #e5e7eb; border-radius: 10px; padding: 16px; margin-bottom: 16px; page-break-inside: avoid; }
            .report-title { font-size: 15px; font-weight: 700; color: #111; margin-bottom: 4px; }
            .report-sub { font-size: 12px; color: #555; margin-bottom: 10px; }
            .badge { display: inline-block; padding: 2px 10px; border-radius: 999px; font-size: 11px; font-weight: 700; }
            .result { font-size: 13px; color: #333; margin-top: 10px; border-top: 1px solid #f0f0f0; padding-top: 10px; }
            .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 10px; }
            .metric { background: #f9fafb; border-radius: 8px; padding: 10px; }
            .metric-label { font-size: 10px; color: #888; font-weight: 700; text-transform: uppercase; }
            .metric-value { font-size: 16px; font-weight: 800; color: #16a34a; margin-top: 2px; }
            @media print { body { padding: 0; } }
          </style>
        </head>
        <body>
          <h1>🌿 AgriMind AI — ${ar ? 'تقارير المنصة' : 'Platform Reports'}</h1>
          <div class="meta">
            ${ar ? 'تاريخ التصدير:' : 'Exported:'} ${new Date().toLocaleString(ar ? 'ar-EG' : 'en-US')}
            &nbsp;·&nbsp; ${ar ? 'عدد التقارير:' : 'Reports:'} ${selectedReports.length}
            &nbsp;·&nbsp; ${ar ? 'المستخدم:' : 'User:'} ${user?.name || 'N/A'}
          </div>
          ${selectedReports.map(r => `
            <div class="report">
              <div class="report-title">${ar ? r.title : r.titleEn}</div>
              <div class="report-sub">${ar ? r.subtitle : r.subtitleEn}</div>
              <div class="result">${ar ? r.result : r.resultEn}</div>
              <div class="grid">
      ...
      if (win) {
        ...
        a.href = url; a.download = `agrimind-reports-${new Date().toISOString().slice(0,10)}.html`;
        a.click();
